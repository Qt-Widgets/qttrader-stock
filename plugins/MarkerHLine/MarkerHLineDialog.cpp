...
MarkerHLineDialog::MarkerHLineDialog (QWidget *p) : QDialog (p)
{
  _settings = 0;
  _modified = 0;
  _helpFile = "main.html";
...
MarkerHLineDialog::loadSettings ()
{
  ...
  QSize sz = settings.value("marker_hline_dialog_size", QSize(200,200)).toSize();
  resize(sz);

  // restore the position of the app
  QPoint p = settings.value("marker_hline_dialog_pos").toPoint();
  if (! p.isNull())
    move(p);
}
...
MarkerHLineDialog::saveSettings ()
{
  QSettings settings(g_settings);
  settings.beginGroup(g_session);
  settings.setValue("markerh_line_dialog_size", size());
...
MarkerHLineDialog::setGUI (Entity *settings)
{
  ...
  if (! set)
  {
    qDebug() << "MarkerHLineDialog::setGUI: no color setting";
    return;
  }
  ...
  if (! set)
  {
    qDebug() << "MarkerHLineDialog::setGUI: no price setting";
    return;
  }
...
void MarkerHLineDialog::valueChanged ()
{
  _modified = 1;
}
