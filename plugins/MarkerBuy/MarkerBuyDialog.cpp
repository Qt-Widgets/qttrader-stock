...
MarkerBuyDialog::MarkerBuyDialog (QWidget *p) : QDialog (p)
{
  _settings = 0;
  _modified = 0;
...
MarkerBuyDialog::loadSettings ()
{
  ...
  QSize sz = settings.value("markerbuy_dialog_size", QSize(200,200)).toSize();
  resize(sz);

  // restore the position of the app
  QPoint p = settings.value("markerbuy_dialog_pos").toPoint();
  if (! p.isNull())
    move(p);
}
...
MarkerBuyDialog::saveSettings ()
{
  QSettings settings(g_settings);
  settings.beginGroup(g_session);
  settings.setValue("markerbuy_dialog_size", size());
  settings.setValue("markerbuy_dialog_pos", pos());
}

void
MarkerBuyDialog::setGUI (Entity *settings)
{
  _settings = settings;
  ...
  if (! set)
  {
    qDebug() << "MarkerBuyDialog::setGUI: no color setting";
    return;
  }
  ...
  if (! set)
  {
    qDebug() << "MarkerBuyDialog::setGUI: no date setting";
    return;
  }
  ...
  if (! set)
  {
    qDebug() << "MarkerBuyDialog::setGUI: no price setting";
    return;
  }
...
void MarkerBuyDialog::valueChanged ()
{
  _modified = 1;
}
